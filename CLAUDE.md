...
8. **コピー/切り取り/貼り付け** ✅
   - Ctrl+C でノードとその子孫をコピー
   - Ctrl+X で切り取り（即座にノードが削除される）
   - Ctrl+V で選択ノードの子として貼り付け
   - 外部アプリとの互換性（Workflowy等）
     - HTML形式（ul/li）で階層構造をエクスポート/インポート
     - インデント付きテキストからも階層を復元
   - Ctrl+Shift+M でMiro用にコピー（テーブル形式）
     - Miroに貼り付けると「Table/Grid/Sticky notes」選択ダイアログが表示
   - 複数選択時は全選択ノードをコピー/切り取り
   - 複数回ペースト可能
...
| Shift+Enter | 上に兄弟ノード作成 | ルートノードでは無効。作成後、編集モードに入る |
| E / F2 | 編集開始 | ダブルクリックでも可 |
| I | アイコンピッカーを開く | 独立ダイアログ、ノード選択しながら使用可能 |
| Ctrl+C | コピー | 選択ノードとその子孫をクリップボードにコピー |
| Ctrl+X | 切り取り | 選択ノードとその子孫を切り取り（ルートは不可） |
...
| Shift+Enter | 上に兄弟ノード作成 | テキストを保存し、上に新しい兄弟ノードの編集を開始（ルートでは無効） |
| Escape | 保存して編集終了 | |
...
| Ctrl+C | コピー | 選択ノードとその子孫をコピー |
| Ctrl+X | 切り取り | 選択ノードとその子孫を切り取り |
| Ctrl+V | 貼り付け | 選択ノードの子として貼り付け |
| Ctrl+Shift+M | Miro用にコピー | テーブル形式でコピー（Miroで貼り付けダイアログが表示） |

### クリップボード操作

- **コピー（Ctrl+C）**: 選択中のノードとその全ての子孫をクリップボードにコピー。複数選択時は全ての選択ノードをコピー
- **切り取り（Ctrl+X）**: コピーと同様だが、貼り付け時に元のノードが削除される。ルートノードは切り取り不可
- **貼り付け（Ctrl+V）**: 選択中のノードの子として貼り付け。階層構造は保持される
- **Miro用にコピー（Ctrl+Shift+M）**: 選択ノードをテーブル形式（TSV + HTML table）でコピー。Miroに貼り付けると「Table/Grid/Sticky notes」選択ダイアログが表示される
- **外部テキストの貼り付け**: アプリ外でコピーしたプレーンテキストも貼り付け可能（新しい子ノードとして作成）
- 貼り付けたノードには新しいIDが割り当てられ、位置は自動計算される
...
| 20 | パン・ズーム | マウスドラッグでパン、ホイールでズーム、Ctrl+ホイールで垂直パン、Shift+ホイール/水平ホイールで水平パン | ✅ |
...
| 29 | コピー/切り取り/貼り付け | Ctrl+C/X/Vでノード操作 | ✅ |
